...
import (
	...
	pbconnect "github.com/hashicorp/consul/agent/agentpb/connect"
...
func (ac *AutoConfig) updateTLSCertificatesInConfig(opts AutoConfigOptions, resp *agentpb.AutoConfigResponse) error {
...
func (ac *AutoConfig) updateACLsInConfig(opts AutoConfigOptions, resp *agentpb.AutoConfigResponse) error {
	acl := &config.ACL{
...
func (ac *AutoConfig) updateJoinAddressesInConfig(opts AutoConfigOptions, resp *agentpb.AutoConfigResponse) error {
	...
	if resp.Config.Gossip == nil {
		resp.Config.Gossip = &config.Gossip{}
	}
...
func (ac *AutoConfig) updateGossipEncryptionInConfig(_ AutoConfigOptions, resp *agentpb.AutoConfigResponse) error {
	...
	if lanKeyring := memberlistConfig.Keyring; lanKeyring != nil {
		...
		if resp.Config.Gossip.Encryption == nil {
			resp.Config.Gossip.Encryption = &config.GossipEncryption{}
		}
...
func (ac *AutoConfig) updateTLSSettingsInConfig(_ AutoConfigOptions, resp *agentpb.AutoConfigResponse) error {
	if ac.tlsConfigurator == nil {
		// TLS is not enabled?
		return nil
	}
...
func (ac *AutoConfig) baseConfig(opts AutoConfigOptions, resp *agentpb.AutoConfigResponse) error {
	if opts.NodeName == "" {
...
type autoConfigUpdater func(c *AutoConfig, opts AutoConfigOptions, resp *agentpb.AutoConfigResponse) error
...
func mapstructureTranslateToProtobuf(in interface{}, out interface{}) error {
	decoder, err := mapstructure.NewDecoder(&mapstructure.DecoderConfig{
		DecodeHook: agentpb.HookTimeToPBTimestamp,
		Result:     out,
	})
