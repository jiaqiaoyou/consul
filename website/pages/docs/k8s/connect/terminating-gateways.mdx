...
```shell-session
curl --request PUT --data @external.json -k http://localhost:8500/v1/catalog/register
```
If ACLs and TLS are enabled :
```shell-session
curl --request PUT --header "X-Consul-Token: $CONSUL_HTTP_TOKEN" --data @external.json -k https://localhost:8501/v1/catalog/register
```

### Update terminating ACL token if ACLs are enabled
...
```
~> If ACLs are enabled a `CAFile` must be provided, it must point to the system trust store of the terminating gateway
container.
...
Finally define and deploy the external services as upstreams for the internal mesh services that wish to talk.
