...
```shell-session
$ export CONSUL_HTTP_TOKEN=$(kubectl get secret consul-bootstrap-acl-token --output=jsonpath={.data.token} | base64 --decode)
```
...
```shell-session
$ kubectl exec static-client-7cc6df495-qdtkp -- curl -vvvs --header "Host: example-https.com" http://localhost:1234/
```
